import React from "react";
import {
  NavWrapper,
  SearchWrapper,
  NavSearch,
  Button,
  Addition
} from './style'

class NavProfile extends React.Component{
    render() {
        return (
          <NavWrapper>
            <SearchWrapper>
              <span>Logo</span><NavSearch></NavSearch>
            </SearchWrapper>
            <Addition>
            <img class='notif'src="https://image.flaticon.com/icons/svg/2097/2097743.svg"></img>
              <img className='pic' src="data:image/png;base64,"></img>
                <Button>Log Out</Button>

            
              
            </Addition>
          </NavWrapper>
    );
  }
}
  
  export default NavProfile;

  /*
              
              <Button> <button class="enter"> bell icon </button> 
                
              <button class= "archive"> log out</button>
              </Button>*/