...
async function monster(link) {
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      title = document.querySelector("h1.job_title").innerHTML;
      company = document.querySelector("div.job_company_name").innerHTML;
      place = document.querySelector("div.location").innerHTML;
      description = document.querySelector("div[name='sanitizedHtml']")
        .innerHTML;
...
async function google(link) {
  try {
    ...
    const data = await page.evaluate(() => {
      title = document.querySelector("h2.KLsYvd").innerHTML;
      company = document.querySelector("div.nJlQNd").innerHTML;
      place = document.querySelectorAll("div.sMzDkb")[1].innerHTML;
      description = document.querySelector("span.HBvzbc").innerHTML;
      return { title, company, place, description };
    });
    console.log(data);
    return data;
